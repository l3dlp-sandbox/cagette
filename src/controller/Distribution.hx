...
class Distribution extends Controller {
	...
	function doInviteFarmers(distrib:db.MultiDistrib) {
		...
		if (cproVendors.length > 0 || invitationsSent.length > 0) {
			...
			for (i in invitationsSent)
				html += '<div class="disabled">${i.name} : <b>invitation envoyée</b></div>';
	...
	**/
	...
	public function doInsertMdCycle() {
		...
		var x = new form.CagetteDatePicker("endDate", "Date de fin de cycle", DateTool.now().deltaDays(30).setHourMinute(23, 59), NativeDatePickerType.date,
			true);
		form.addElement(x, 4);
	...
	**/
							...
							try {
								...
								if (data.ResultMessage == "Transaction has already been successfully refunded") {
									...
									if (o.getPaymentData().remoteOpId != Std.string(payinToRefundId)) {
										throw new tink.core.Error("found refund do not relate to payin. mgp error :" + data + ", payin id "
											+ o.getPaymentData().remoteOpId);
									}
									for (mgpRefund in Mangopay.getPayInRefunds(payinToRefundId)) {
										if (mgpRefund.Nature != "REFUND" || mgpRefund.Status != "SUCCEEDED")
											continue;
	...
	public function doAutovalidate(md:db.MultiDistrib) {
		checkHasDistributionSectionAccess();
		if (md.isValidated())
			throw Redirect("/distribution/validate/" + md.id);

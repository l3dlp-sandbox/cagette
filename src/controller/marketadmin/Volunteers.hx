package controller.marketadmin;
...
{
	@tpl("marketadmin/volunteers/default.mtt")
	function doDefault() {
		...
		if (form.isValid()) {
			...
			try {
				...
				throw Error("/marketadmin/volunteers", e.message);
			}			
			...
			throw Ok("/marketadmin/volunteers", t._("Your changes have been successfully saved."));
	...
	**/
	...
	function doInsertRole() {
		...
		if (form.isValid()) {
			...
			role.insert();
			throw Ok("/marketadmin/volunteers", t._("Volunteer Role has been successfully added"));
	...
	function doEditRole(role:db.VolunteerRole) {
		...
		if (form.isValid()) {
			...
			throw Ok("/marketadmin/volunteers", t._("Volunteer Role has been successfully updated"));
	...
	function doDeleteRole(role: db.VolunteerRole, args: { token:String , ?force:Bool}) {
		...
		if ( checkToken() ) {
			...
			throw Ok("/marketadmin/volunteers", t._("Volunteer Role has been successfully deleted"));
		} else {
			throw Redirect("/marketadmin/volunteers");
		}
	}
